          ...
          <div class="form-row column">
            ...
            <div class="button-group">
              <button
                  ...
                  @click="useDeepThinking = false"
                  type="button"
                  :data-tooltip="useDeepThinking ? '点击取消使用深度思考' : '更快速'"
              >
                <svg
              ...
              <button
                  :class="['deep-think-btn', { active: useDeepThinking }]"
                  @click="useDeepThinking = true"
                  type="button"
                  :data-tooltip="useDeepThinking ? '已启用深度思考' : '点击启用深度思考'"
              >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="1em"
              ...
              <div class="bubble-top">
                <div class="bubble-top-row">
                  ...
                  <div class="right-controls">
                    <button v-if="q.isDeepThinking" v-show="!q.collapsedAll" class="collapse-btn" @click="toggleCollapse(index)">
                      {{ q.collapsedDeepThinking ? '展开深度思考' : '折叠深度思考' }}
                    </button>
                    <button class="collapse-btn" @click="toggleCollapseAll(index)">
                      {{ q.collapsedAll ? '展开回答' : '折叠回答' }}
                    </button>
                    <div class="create-time">{{ q.creatTime }}</div>
                  </div>
                </div>
                <div class="divider" v-show="!q.collapsedDeepThinking && !q.collapsedAll && q.isDeepThinking"></div>
                <div class="ai-response deepthink-response" v-show="!q.collapsedDeepThinking && !q.collapsedAll && q.isDeepThinking">{{ q.deepThinkingResponse }}</div>
                <div v-show="!q.collapsedAll" class="divider"></div>
                <div v-show="!q.collapsedAll" class="ai-response">{{ q.chatResponse }}</div>
...
export default {
  ...
  methods: {
    ...
    toggleCollapseAll(index) {
      this.questions[index].collapsedAll = !this.questions[index].collapsedAll;
    },
    confirmDelete(index) {
